...
public class SimpleNUnit
{
    ...
    /// <summary>
    /// Runs all tests found in DLLs in the specified directory and returns results.
    /// </summary>
    /// <param name="path">Path to the directory containing test assemblies.</param>
    /// <param name="progress">Optional progress reporter that receives each finished test result.</param>
    /// <returns>All collected test results.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown if required test attributes or their properties are not found.
    /// </exception>
    /// <exception cref="ArgumentException">Thrown if <paramref name="path"/> is null/empty/whitespace.</exception>
    public async Task<IReadOnlyList<TestResult>> RunAsync(string path, IProgress<TestResult>? progress = null)
    {
